...
> **목표**: GC Pause 4.6초 → 1초 미만, 쓰기 포화 처리량 480~490/s → 개선 측정
...
> 🚨 **Before/After 설계 주의 (2026-07-29 추가)** — Before 기준선 480~490/s는 **10~15초 구간 실측치**다
> (Day 7 런 길이 VU별 30/15/15/10/10초). **After를 더 긴 런으로 측정해 그대로 비교하면 튜닝 효과가
> 아니라 측정 창 차이를 재는 것이 된다.** 개선 측정 시 (a) After도 동일한 15초 구간으로 맞추거나,
> (b) Before를 지속 런으로 다시 뜨거나 둘 중 하나를 먼저 정할 것.
> ⚠️ 지속 런으로 갈 경우 **XXL 유저풀(10,000) 소진**이 걸린다 — 490/s × 60s ≈ 29,400 > 10,000이라
> 약 20초부터 409가 지배한다. 선행 조치는 `docs/fix-instructions/09-loadtest-write-heavy-유저풀-소진-지뢰.md`.
...
|--------|-----------------|-----------------|------|
| **측정 창** | 2m0s (지속) | **10~15s (구간)** | ⚠️ 조건 상이 — 아래 TPS 행 비교 시 전제 |
| **TPS** | 912 req/s (GET 전용) | **480~490/s** | 쓰기 = 읽기의 약 54% (단위 상이 — 참고치) |
...
- 쓰기 포화 처리량: 480~490/s → 550~600/s (가설, 커넥션 대기 감소분 — **동일 측정 창 기준으로 비교할 것**)
...
> ⚠️ **측정 창 열을 반드시 채울 것** — Before는 15초 구간이다. After를 다른 길이로 재면 비교가 무효다.

| 튜닝 단계 | 측정 창 | TPS (write) | p95 | GC Pause max | Pending max | CPU max |
|-----------|--------|------------|-----|-------------|-------------|---------|
| **Before (현재)** | **15s 구간** | 480~490 | 180ms | 4.6초 | 140 | 85% |
| After Step 1 (GC) | ? | ? | ? | ? | ? | ? |
| After Step 2 (Pool) | ? | ? | ? | ? | ? | ? |
| After Step 3 (Scale) | ? | ? | ? | ? | ? | ? |
