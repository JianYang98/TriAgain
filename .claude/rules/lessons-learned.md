...
- 재발: 3회 (2026-08-11, e2eTest 0건 가드) — 대조군을 두 번 다 잘못 만들었다. ① init 스크립트의 `includeTags` 가 덮어쓰기가 아니라 합쳐져(OR) 원래 7클래스가 그대로 돌았다 ② 가드만 빼려던 편집이 Groovy 문법을 깨 **다른 이유로** 실패했다. ⚠️ 이 파일은 `src/**/*.java` 편집 때만 로드되는데 사고는 `build.gradle` 에서 났다 — 로드 조건 재검토 대상
