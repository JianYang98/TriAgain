...
- 사례: 런북·지시서에 프로드 RDS를 "db.t3.micro / PostgreSQL 16 / gp3"로 기재했으나, 2026-07-12 콘솔 실측 결과 실물은 "db.t4g.micro(ARM/Graviton) / PostgreSQL 17.9 / gp2". 부하테스트 목적이 "프로드 재현"이라 스펙이 틀리면 재현 전제가 어긋난다(예: pg_dump 클라이언트 버전 요구, ARM↔x86 버스트 거동).
- 왜?: 런북 작성 시 프로드 RDS 스펙을 콘솔/CLI로 확인하지 않고 기억·추정으로 기재 → 문서 드리프트. "EC2가 t3.micro니 RDS도 당연히 db.t3.micro겠지" 식의 유추가 근본 원인.
- 규칙: 인프라 값(인스턴스 클래스, 엔진 버전, 스토리지 타입, AZ/VPC 등)을 문서에 적을 땐 그 시점에 콘솔/CLI로 실측하고 값 옆에 "(YYYY-MM-DD 실측)" 근거를 남긴다. 실측 못 하면 "[미확인]"으로 명시하고 추정값을 확정값처럼 쓰지 않는다.
