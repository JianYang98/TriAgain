...
- 지시서/SDD가 이미 3문을 판정한 조각이면, 재판정 결과가 **지시서와 다를 때 그 사유를 반드시 기록한다.** 조용히 뒤집으면 아무도 못 잡는다 (2026-07-30 실사례: 지시서가 "에러코드 정책을 빼라"고 명시한 조각을 구현이 "껍데기"로 재분류)
  - **유지로 뒤집었으면** → 위 마커에 적는다 (코드에 남으므로 다음 리뷰가 본다)
  - **추출로 뒤집었으면** → 마커가 사라지니 적을 자리가 없다. **PR 본문 또는 `docs/harness-decisions.md`** 에 적는다. 지시서가 "유지"의 근거로 든 제약(성능·시그니처 차이 등)이 실재했다면 그걸 버린 이유가 남아야 한다
- 재발: 1회
...
### 단위테스트의 관측 지점이 프레임워크 내부 순서 때문에 대상과 무관하게 참일 수 있다
- 출처: upload-session-status-auth /implement (2026-08-23, impl-guards.md G-8)
- 사례: `SseEmitterAdapterTest`에 `onTimeout`의 `emitter.complete()` 호출을 검증하는 테스트를 추가하고, `complete()` 유무에 따라 이후 `send()`가 `IllegalStateException`을 던지는지로 단언했다. 변이(M6, `complete()` 삭제)를 돌려도 GREEN이 유지됐다 — `ResponseBodyEmitter$DefaultCallback.run()`이 우리 델리게이트를 실행하기 **전에** `complete=true`를 먼저 세팅한다는 걸 바이트코드로 확인(spring-webmvc-6.2.15). 즉 그 단언은 `complete()` 호출 여부와 무관하게 항상 참이었다.
- 왜?: `ResponseBodyEmitter.Handler`가 package-private이라 `com.triagain.*`에서 주입·모킹이 불가능하고, `complete()`의 유일한 추가 효과(`handler.complete()` 호출)는 실제 서블릿 비동기 컨텍스트가 붙어야만 관측된다. 순수 단위테스트가 고를 수 있는 대체 관측 지점(`send()` 예외)은 Spring 내부 필드가 우리 코드보다 먼저 세팅해버려 판별력이 없었다.
- 규칙: 프레임워크 내부 상태(필드 실행 순서·package-private 콜백)에 얽힌 효과를 단위테스트로 검증하려 할 땐, 그 관측 지점이 대상 코드와 무관하게 참이 될 수 있는지 먼저 의심한다. 필요하면 프레임워크 소스(바이트코드/decompile)로 실행 순서를 확인하고, 원리적으로 격리 불가능하면 실제 서블릿 비동기 계층을 태우는 `*ApiTest`(RANDOM_PORT)로 검증을 옮긴다.
- 형제 항목: **"실패 모드를 문서에 적을 땐 실제로 깨뜨려 보고 적는다"**(바로 위) — 같은 도구(변이)로 검증했기 때문에 공허함이 발각됐다는 점에서 연장선이지만, 그쪽은 "검증 자체를 안 했다"가 원인이고 이쪽은 "검증했는데 그 관측 지점이 프레임워크 내부 순서 때문에 원리적으로 무의미했다"가 원인이라 처방이 다르다.

### 변이-테스트 대응표는 목이 변이 지점을 가로막지 않는지까지 확인한다
- 출처: upload-session-status-auth /implement (2026-08-23, impl-guards.md G-1 표 정정)
- 사례: SDD가 소유권 우회 변이(M2: `UploadSessionQueryService.getOwnedOrThrow` 내부 구현을 `findById`로 바꿔치기)의 적색 대상으로 `SubscribeUploadSessionServiceTest.S4`를 지목했다. 그러나 S4는 `UploadSessionQueryUseCase` **인터페이스를 목으로 잡는** 단위테스트라 그 뒤의 구현체(`UploadSessionQueryService`) 내부 변이는 원리적으로 감지할 수 없다 — 실측 결과 M2 적용 후에도 S4는 GREEN을 유지했다(결함이 아니라 대응표 오류).
- 왜?: 대응표를 짤 때 "테스트 이름이 그 시나리오를 말하는가"만 보고 "그 테스트가 변이가 심긴 코드를 실제로 호출·통과하는가"(모킹 경계가 그 지점을 가로막지 않는가)는 확인하지 않았다.
- 규칙: 변이-테스트 대응표의 각 행마다 "변이가 심는 코드"와 "테스트가 그 코드를 실제로 경유하는가"를 먼저 추적한다. 인터페이스를 목으로 잡는 테스트는 그 인터페이스 **구현체 내부** 변이에 원리적으로 둔감하다 — 구현체 변이의 적색 대상은 그 구현체를 직접 부르는 테스트(통합/API 레벨)로 잡는다. 이름이 맞아도 관점(무엇을 목으로 잡았는가)이 틀릴 수 있다.
