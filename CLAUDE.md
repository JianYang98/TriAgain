...
>
> ⚠️ **아직 규칙이 붙지 않은 영역**: Flyway 마이그레이션(`src/main/resources/db/migration/*.sql`),
> `application*.yml`, `Dockerfile`, `.github/workflows/deploy.yml`. 전부 tier-policy Tier 3 대상인데
> 로드되는 규칙이 0개다 — 이 영역에서 실수가 나오면 **위 표의 서버 파일이 아니라 `db-migration` 등
> 전용 규칙 신설을 먼저 검토**한다(루트 `docs/harness-decisions.md` 2026-07-27).
