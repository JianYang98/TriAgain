...
Java 8부터 **람다 표현식**으로 한 줄로 줄일 수 있다:
...
**파일:** `crew/domain/model/Crew.java`
...
**파일:** `crew/application/GetMyCrewsService.java`
...
**파일:** `crew/infra/CrewJpaAdapter.java`
...
`클래스명::메서드명` 은 `(인스턴스) -> 인스턴스.메서드()` 의 축약형이다.

#### (4) forEach — "각 요소에 대해 작업 수행"

**파일:** `crew/application/ActivateCrewService.java`

```java
crew.getMembers().forEach(member -> {
    Challenge challenge = Challenge.createFirst(
            member.getUserId(), crew.getId(), startDate, deadline
    );
...
**파일:** `crew/api/JoinCrewRequest.java`
...
**파일:** `crew/api/CreateCrewRequest.java`
...
**파일:** `common/response/ErrorResponse.java`
...
**null을 안전하게 다루는 래퍼 클래스**다. "값이 있을 수도 있고, 없을 수도 있다"를 타입으로 표현한다.
...
**파일:** `user/application/GetUserService.java`
...
**파일:** `moderation/infra/VerificationClientAdapter.java`
...
**파일:** `verification/infra/ChallengeClientAdapter.java`
...
```
O 반환 타입으로 사용:    Optional<User> findById(String id)
X 필드로 사용 금지:      private Optional<String> name
X 매개변수로 사용 금지:   void save(Optional<User> user)
```
