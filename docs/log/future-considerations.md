...
- 현재 상태: `MethodLength`는 **여는 중괄호 줄부터 닫는 중괄호 줄까지**만 세고, `countEmpty=false`라 그 구간에서 빈 줄·주석 줄을 뺀다. 시그니처가 여러 줄이면 그 줄들은 안 세어진다. `Crew.create`는 가이드라인 34줄 / checkstyle 28줄인데, **차이 6줄이 전부 시그니처**다(이 메서드엔 주석 0줄).
- 필요 시점: 지금 — 별도 PR에서 `CLAUDE.md` 문구를 도구에 맞춘다 (하네스 변경 트랙)
- 이유: 사람과 도구가 다른 답을 내면 리뷰에서 반복해 부딪힌다(PR #126에서 실제로 4파일). `countEmpty`를 뒤집어도 시그니처 제외는 그대로라 **설정으로는 못 고친다** — 문구를 도구에 맞추는 쪽만 남는다.
