...
- GET 상태 조회와 SSE 구독은 둘 다 인증이 필요하고 요청자 소유 세션만 접근할 수 있다.
  타인 소유·부재 세션은 `404 V004`로 응답해 세션 id 존재 여부를 흘리지 않는다.
...
| 완료 알림 계약 | SSE와 2초 폴링 병행 | SSE 연결 실패·유실 시에도 완료 확인 가능 | 중복 조회 트래픽과 두 경로의 유지 비용 발생 |
