...
- `nickname`은 2~12자의 한글·영문·숫자·언더스코어만 허용한다. **앞뒤 공백은 트림한 뒤 검증·저장한다** — `"  닉네임  "`은 `"닉네임"`으로 가입된다. (`PATCH /users/me/nickname`은 반대로 앞뒤 공백을 400으로 거부한다.)
...
| 400 | U007 | 2~12자 한글·영문·숫자·언더스코어 규칙 불일치. **앞뒤 공백도 트림 없이 거부된다** — 가입 경로가 트림하는 것과 다르다 |
